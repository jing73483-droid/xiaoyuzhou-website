...
(()=>{
 ...
 const aura=ctx.createRadialGradient(cx,cy,rad*.22,cx,cy,rad*1.55);aura.addColorStop(0,'#13232200');aura.addColorStop(.43,'#42ffc20b');aura.addColorStop(.67,'#714cff25');aura.addColorStop(1,'#06080e00');ctx.fillStyle=aura;ctx.fillRect(0,0,width,height);
 for(const s of stars){const x=(s.x*width+time*2*s.size)%width;ctx.fillStyle=`rgba(155,205,211,${.15+s.size*.2})`;ctx.fillRect(x,s.y*height,s.size,s.size);}
 ...
 sorted.sort((a,b)=>a.z-b.z);for(const p of sorted){const light=Math.max(.18,.55+p.z/(rad*1.5));ctx.fillStyle=p.z>0?`rgba(132,255,204,${light})`:`rgba(141,105,255,${light})`;ctx.beginPath();ctx.arc(p.x,p.y,p.size,0,Math.PI*2);ctx.fill();}
 ctx.save();ctx.translate(cx,cy);ctx.rotate(-.35+mx*.04);for(let ring=0;ring<3;ring++){ctx.strokeStyle=ring===0?'#b5ffaf38':'#8971ff30';ctx.lineWidth=ring===0?1:.6;ctx.beginPath();ctx.ellipse(0,0,rad*(1.1+ring*.10),rad*(.73+ring*.03),ring*.11,0,Math.PI*2);ctx.stroke();}for(let i=0;i<70;i++){const a=i/70*Math.PI*2+time*.045,inner=rad*1.26,outer=inner+(i%5===0?8:3);ctx.strokeStyle=i%5===0?'#9bf5cc80':'#87a1b940';ctx.beginPath();ctx.moveTo(Math.cos(a)*inner,Math.sin(a)*inner*.84);ctx.lineTo(Math.cos(a)*outer,Math.sin(a)*outer*.84);ctx.stroke();}ctx.restore();
 }
