...
(()=>{
 ...
 function sync(){root.classList.toggle('film-paused',paused||document.hidden);toggle.textContent=paused?'播放漫游':'暂停漫游';toggle.setAttribute('aria-label',paused?'播放自动漫游':'暂停自动漫游');toggle.setAttribute('aria-pressed',String(!paused));caption.textContent=titles[index];count.textContent=String(index+1).padStart(2,'0')+' / 04';}
 ...
 window.addEventListener('world-arrived',e=>{index=e.detail.index;elapsed=0;transitioning=false;bar.style.transform='scaleX(0)';scenes.forEach((s,i)=>s.classList.toggle('film-shot',i===index));sync();});
